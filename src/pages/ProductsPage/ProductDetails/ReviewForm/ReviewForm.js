...
const ReviewForm = ({ productId }) => {
  ...
  const onSubmit = async (data) => {
    ...
    const productsReponse = await axios.get(`http://localhost:3100/products/${productId}`)
    const product = productsReponse.data
    const updateProductReviews = {
      ...product,
      reviews: [...product.reviews, newreviews]
    }
    console.log('updated Product Reviews', updateProductReviews)

    await axios.post('http://localhost:3100/products', updateProductReviews, {
      headers: {
